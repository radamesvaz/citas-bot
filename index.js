...
const runBot = async () => {
    ...
    try {
        ...
        console.log(colors.bg.blue, colors.fg.white, "Iniciando...", colors.reset) ; 
    } catch (e) {
        console.log(colors.bg.red, colors.fg.white, "Error:", colors.reset) ; 
        console.log(e);
        await browser.close();
        runBot();
    }
    ...
    const checkForAppointment = async () => {
        ...
        if (options.length !== 2){
            for(let i = 0; i < 20; i++ ) {
                console.log(colors.bg.yellow, colors.fg.crimson, "YA ESTA LA OPCION", colors.reset) ; 
                console.log(options)
            } 
        } else {
            console.log(colors.bg.green, colors.fg.white, "Volviendo a intentar en 27 minutos", colors.reset) ; 
            console.log(options)
            setTimeout(async () => {
                await browser.close();
                runBot();
